...
| | Troubleshooting - 1. Use Extended Events or SQL Trace  </br> 2. Execute and test the queries in SQLCMD or in SQL Server Management Studio (SSMS). 3. Increase timeout time 4. Split query into smaller parts |   
...
<details> 
  <summary>
   SQL Server on github is slightly difficult. 
   EASY TO EXPLAIN SOLN -> 
                                 SELECT salary FROM employee ORDER BY salary DESC   LIMIT 2, 1;
   This is equivalent to         SELECT salary FROM employee ORDER BY salary DESC   LIMIT 1 OFFSET 2;   
