...
Scalability refers to a system's ability to handle increased load or demand by adding resources. In simpler terms, it's how well a system can grow and still perform efficiently as usage grows.

There are two main types:

*  Vertical scalability (scale up): Increasing the capacity of a single server (e.g., more CPU, RAM).

*  Horizontal scalability (scale out): Adding more servers or nodes to distribute the load.

A scalable system maintains performance, availability, and reliability even as the user base or data grows.
...
Performance refers to how efficiently a system completes tasks and responds to user actions. In software systems, it typically focuses on:

*    Response time: How quickly the system responds to a request.

*    Throughput: How many requests the system can handle in a given time (e.g., requests per second).

*    Latency: The delay between a request and the start of the response.

*    Resource usage: How much CPU, memory, or bandwidth the system consumes while working.

Good performance means fast, responsive, and resource-efficient systems that provide a smooth user experience even under load.
...
Availability refers to the ability of a system to remain accessible and operational over time. It's usually expressed as a percentage (e.g., 99.9% availability means the system is down for less than 9 hours per year).

Key aspects include:

*    Uptime: The amount of time the system is functioning correctly.

*    Fault tolerance: The system’s ability to keep working even when some components fail.

*    Redundancy: Having backup components (servers, networks, databases) to ensure continuity.

High availability is critical for systems that need to be reliable and always online, such as banking apps, e-commerce platforms, or cloud services.
</details>
