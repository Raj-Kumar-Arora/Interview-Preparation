...
|                  |  e.g. you can assign any of the following three values to a bool? variable: true, false, or null.           |
| Null-coalescing  |  https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/null-coalescing-operator      |
|    operators     |  ??   Returns the value of its left-hand operand if it isn't null; **otherwise**,                           |
|                  |       it evaluates the right-hand operand and returns its result.                                           |
|   ?? and ??=     |  ??=  Assigns the value of its right-hand operand to its left-hand operand **only if**                      |
