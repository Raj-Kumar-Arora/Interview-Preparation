...
| **For LINQ To Objects ** (local collections), Func<T, U> is passed to LINQ.                <br/> 
                   IEnumerable<T> is the main interface which is base for LINQ queries.      |
| **For LINQ to Entities** (collections in Database),                                        <br/> 
    Expression <Func<T, U>> is passed to LINQ and further converted to actual Database Query <br/> 
    IQuerable <T> is the main interface which is on top of IEnumerable<T> to provide extra methods. <br/> 
...
| (REF - What is the difference between IEnumerable and IQueryable in C#? |
